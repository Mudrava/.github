## MUDRAVA - Boutique IT Studio

**Fixing the complex. Building for scale.**

We're Vadym (Chief Architect, 18 yrs) and Anastasia (Lead Designer, 11 yrs) - a two-person studio that rescues failed software projects and builds scalable MVPs for funded startups.

### What we do
- **Project Rescue** - we step into failed MVPs and ship them
- **MVP from Scratch** - enterprise-grade architecture, 8-10 week launch
- **Fractional CTO** - CTO-level expertise, 10-20 hrs/week
- **UX/UI Redesign** - conversion-focused, not just pretty

### Our open-source tools
- [`pixelfixer-mcp-server`](https://github.com/Mudrava/pixelfixer-mcp-server) - MCP server for AI-powered bug tracking
- [`True-RUM-Monitor`](https://github.com/Mudrava/True-RUM-Monitor) - Real User Monitoring plugin for WordPress
- [`WP-Mail-to-Telegram`](https://github.com/Mudrava/WP-Mail-to-Telegram) - Forward WordPress emails to Telegram

### Links
🌐 [mudrava.com](https://mudrava.com/en/) · 📅 [Book a call](https://calendly.com/mudrava) · 💼 [Upwork](https://www.upwork.com/agencies/mudrava)
